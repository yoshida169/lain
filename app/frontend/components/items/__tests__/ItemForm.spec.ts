...
describe('ItemForm', () => {
  ...
  it('initial が渡された場合は初期値が入力される', () => {
    ...
    expect((wrapper.find('#item-title').element as HTMLInputElement).value).toBe('既存タイトル')
    expect((wrapper.find('#item-content').element as HTMLTextAreaElement).value).toBe('既存コンテンツ')
  })
