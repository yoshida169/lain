...
- [x] `app/frontend/App.vue` を作成（router-view + ヘッダー）
- [x] `app/frontend/router/index.ts` を作成（`/vue/items` 系ルート）
- [x] `app/frontend/api/client.ts` を作成（CSRF 対応 fetch wrapper）
- [x] `app/frontend/api/items.ts` を作成（getItems, getItem, createItem, updateItem, deleteItem）
- [x] `app/frontend/api/tags.ts` を作成（getTags）
- [x] `app/frontend/components/common/AppHeader.vue` を作成
- [x] `app/frontend/components/common/FlashMessage.vue` を作成
- [x] `app/controllers/vue_controller.rb` を作成
- [x] `app/views/vue/index.html.erb` を作成（`<div id="app">` のみ）
- [x] `config/routes.rb` に `get "/vue/*path", to: "vue#index"` を追加
- [x] `app/frontend/entrypoints/application.ts` を更新（Vue マウント）
- [x] 動作確認：`/vue/items` で Vue シェルが表示される
...
- [x] `config/routes.rb` を更新：`/items/*` を Vue controller に向ける
- [x] `app/frontend/router/index.ts` から `/vue` プレフィックスを削除
- [x] ERB ビュー削除：`app/views/items/` 全6ファイル
- [x] `app/controllers/items_controller.rb` を削除
- [x] Stimulus 関連ファイル削除：`app/javascript/` 配下全ファイル
- [x] `config/importmap.rb` を削除
- [x] Gemfile から不要 gem 削除：`importmap-rails`, `turbo-rails`, `stimulus-rails`, `tailwindcss-rails`
- [x] `app/views/layouts/application.html.erb` から Importmap タグ・旧 Tailwind タグ削除
- [x] `bundle install` 実行
- [x] 動作確認：`/items/*` が Vue SPA として動作
